<template>
  <div class="ha-sidebar-wrapper" :class="{'left': position == 'left', 'right': position == 'right'}">
    <div class="click-block click-block-sidebar"
         :class="{'click-block-hide': !opened}"
         @click="close()"
    ></div>
    <scroller width="260" class="ha-sidebar" :class="{'open': opened}">
      <slot></slot>
    </scroller>
  </div>
</template>
    ...
    methods: {
      ...
      wrapper() {
        return document.querySelector(this.wrapperSelector ?
          this.wrapperSelector : '[ha-sidebar-container]')
      }
    }
  }
</script>
