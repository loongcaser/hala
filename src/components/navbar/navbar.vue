<template>
  <transition name="nav-fade">
    <div ha-nav v-show="!hideNavbar" class="navbar">
    ...
    export default {
        ...
        channel.$on('PageTransitionEvent', (data) => {
          if (is_first_render) {
            function setNavbarVisible() {
              document.querySelector('[ha-nav]').classList.add('visible')
            }
            if (!!data.hideNavbar) {
          ...
          let direction = document.querySelector('[gs-app]').getAttribute('transition-direction')
          this.title = data.title ? data.title : ''
          this.hideNavbar = !!data.hideNavbar

          // 浏览器前进、后退按钮被连续点击
          if (document.querySelectorAll('[ha-nav] .center').length > 1) {
      ...
      methods: {
        backButtonClicked(e) {
          ...
          document.querySelector('[gs-app]').setAttribute('transition-direction', 'back')
          history.go(-1)
        },
